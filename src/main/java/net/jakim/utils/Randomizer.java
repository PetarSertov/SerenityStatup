...
public class Randomizer
{
    ...
    public static String randomizeValue( String value )
    {
        ...
        if ( tempValue != null )
        {
            ...
            logger.debug( "After replacing placeholders with random upper case letters: " + tempValue );

            logger.debug( "Replacing placeholders with random lower case letters" );
            tempValue = replaceAllOcurances( tempValue,
                                             REPLACE_WITH_LOWER_CASE_LETTERS );
            logger.debug( "After replacing placeholders with random lower case letters: " + tempValue );

            logger.debug( "Replacing placeholders with random digits" );
            tempValue = replaceAllOcurances( tempValue,
                                             REPLACE_WITH_DIGITS );
            logger.debug( "After replacing placeholders with random digits: " + tempValue );
        }
                                     ...
                                     int length )
    {
        ...
        logger.debug( "Generating random unit with size " + length );
                                               ...
                                               int replaceSeed )
    {
        ...
        logger.debug( "Pattern for matching is build: " + pattern.toString( ) );
        logger.debug( "Matching the provided value with built pattern: " + value );
        ...
        if ( matcher.find( ) )
        {
            ...
            for ( int groupIndex = 0; groupIndex <= matcher.groupCount( ); groupIndex++ )
            {
                ...
                String replacement = randomize( arr,
                                                length );
                logger.debug( "Replacing matched item with: " + replacement );
        ...
        logger.debug( "Exiting replaceAllOcurances() method" );
        return value;
    }
}
